...
| A. 디자인 폴리싱 | 🔄 진행 예정 | 화면별 Figma 캡처 문서 기준으로 diff 수정 |
| B. QA 수정 | 🔄 진행 예정 | 아래 표 |
...
| 온보딩 | 폴리곤 진입점 버튼 별도로 두기 | P1 | 화면 독립 ✅ | 남음 |
...
| 공통 | 텍스트 필드/에디터 키보드 올라올 때 '완료' 버튼이 키보드 위에 붙는 문제 | P0 | ⚠️ 공유 modifier | 남음 |
...
1. **먼저 순차 처리 (공유 파일 충돌원)** — `dev`에 순서대로 머지:
   ...
   - 키보드 '완료' 버튼 문제 (공유 키보드 toolbar modifier)
