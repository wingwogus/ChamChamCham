...
- 기준 브랜치: `dev`
- 구현 브랜치: `feat/report-coaching(front)`
- 사용자 승인 범위: 기록 상세 코칭, 리포트 목록, 작업별 리포트 상세, 그래프, 리포트 코칭

## 배경

현재 iOS 앱의 기록 상세는 `참참참의 코칭`을 정적 준비 중 카드로 표시하고,
`영농 기록 > 리포트` 탭은 전체가 placeholder다. 배포 서버에는 기록 코칭과
재배 주기 기반 리포트 API가 이미 존재하므로, Figma의 기록 코칭·리포트 화면을
실데이터와 연결한다.

초기 조사에서 저장소의 2026-07-15 Swagger 스냅샷만 확인해 API 미배포로
판단했으나, 2026-07-16 라이브 `/v3/api-docs` 확인 결과 관련 API가 배포되어
있음을 확인했다. 구현 시작 전에 라이브 Swagger를 저장소 스냅샷으로 동기화한다.
...
(`StatusResponse`, `DetailResponse`)으로 합쳐지는 이름 충돌이 있다. 예를 들어 기록
코칭 응답과 리포트 코칭 응답이 모두 `ApiResponseStatusResponse`를 가리키며,
리포트 재배 주기 상세와 작업별 상세도 같은 `ApiResponseDetailResponse`를 가리킨다.
클라이언트 DTO는 OpenAPI의 충돌한 스키마 이름을 그대로 복사하지 않고, 실제 엔드포인트
응답을 만드는 백엔드 DTO를 기준으로 서로 다른 이름을 사용한다. 디코딩 fixture 테스트로
각 엔드포인트의 실제 JSON 형태를 고정한다.

## 목표

1. 기록 상세에서 실제 기록 코칭의 전체 상태를 표시한다.
2. 리포트 탭에서 서버의 작업별 리포트를 필터링하고 페이지 단위로 조회한다.
3. 여덟 작업 유형의 통계만 사용해 Figma 그래프와 요약 지표를 구성한다.
4. 완료된 재배 주기의 작업별 코칭을 표시하고 실패·오래됨 상태에서 재생성한다.
5. 네트워크가 없을 때 마지막으로 검증된 읽기 캐시를 표시하되 코칭·통계를 로컬에서
   만들어 내지 않는다.
6. iPhone 13 Figma 기준을 따르면서 iPhone SE 2/3에서도 내용과 동작이 잘리지 않게 한다.
...
`Record`는 기존 기록 CRUD를 유지하고 기록 코칭 전용 타입과 저장소만 추가한다.
리포트는 `Features/Report/{Data,Domain,Presentation}`에 별도 기능으로 둔다.

- `RecordFeedbackRepository`
  - 기록 코칭 조회와 실패 상태 재생성
  - 네트워크 성공 응답 캐시와 캐시 fallback
- `ReportRepository`
  - 작업별 리포트 목록, 작업별 상세, 리포트 코칭 재생성
  - 목록·상세 읽기 캐시와 캐시 fallback
- `RecordDetailViewModel`
  - 기록 상세와 코칭을 독립 상태로 관리
  - 기록 본문은 코칭 실패나 지연과 무관하게 먼저 표시
- `ReportListViewModel`
  - 필터, cursor 페이지네이션, 새로고침, 캐시 여부 관리
- `ReportDetailViewModel`
  - 작업별 상세, 코칭 상태, bounded polling, 재생성 관리

`RecordListView`는 상단 앱 바와 `기록/리포트` 탭을 계속 소유한다. 기존 `[UUID]`
navigation path는 다음 route enum으로 바꾼다.

```swift
enum RecordRoute: Hashable {
    case recordDetail(UUID)
    case reportDetail(reportId: UUID, workType: FarmingWorkType)
    case reportRecordHistory(reportId: UUID, workType: FarmingWorkType)
}
```

리포트 구현이 기존 홈·기록 작성 흐름에 영향을 주지 않도록 기존
...
- 기록 코칭
  - 상태: `PENDING`, `READY`, `FAILED`, `STALE`
  - 성공 내용: 좋은 점 한 개, 다음 행동 2~3개
  - 다음 행동의 `due`, `category` enum은 화면에 한국어로 변환
- 리포트 목록
...
기존 기록 상세 아래쪽 섹션을 유지하되 상태별로 내용을 교체한다.

- loading / PENDING: 연한 배경의 준비 중 카드와 진행 표시
- READY: 좋은 점 badge와 본문, 다음 행동 2~3개 목록
- FAILED: 실패 안내와 `다시 코칭 받기` 버튼
- STALE: 이전 코칭 내용을 숨기고 `기록 변경 내용을 반영하고 있어요` 안내
- offline cache: 마지막 성공 코칭을 표시하고 업데이트 시각·오프라인 표시
- no cache offline: 연결 안내와 재시도 버튼

기록 코칭 재생성 API는 현재 FAILED만 허용하므로 STALE에는 활성 재생성 버튼을
보이지 않는다. PENDING 상태는 화면이 보이는 동안에만 제한적으로 다시 조회하고,
화면 이탈 시 즉시 취소한다.
...
- 필터가 적용되면 chip을 primary border/text로 표시하고 선택 수 또는 선택 이름 표시
- 상태: 최초 loading, cached loading, empty, error/retry, offline/no cache,
  refresh, loading more
- 카드는 `GET /farming-reports/work-items` 결과만 사용하고, 목록에서 코칭 상태를
  얻기 위한 N+1 요청은 하지 않는다.
...
각 배열에 여러 문장이 오면 각 문장을 독립 카드로 반복하지 않고 같은 역할 카드
안에서 문단으로 쌓아 카드 수가 무한히 늘지 않게 한다.
...
reportId 또는 정확한 재배 주기 경계를 받지 않는다. 응답에도 farmId가 없어 같은
작물을 여러 농장에서 재배하면 리포트 소속 기록을 정확히 판별할 수 없다.
...
- 한 작업 유형의 코칭 실패가 목록이나 다른 리포트 상세을 막지 않는다.
...
- 기록 코칭 DTO: PENDING/READY/FAILED/STALE fixture decode
...
2. 기록 코칭 DTO/repository/cache 테스트와 구현
3. 기록 상세 코칭 상태 UI
4. 리포트 domain/DTO/repository/cache 테스트와 구현
5. 리포트 목록·필터·route
6. 작업별 표시 모델과 summary 테스트
7. chart 표시 모델·SwiftUI chart 카드
8. 리포트 상세·코칭·기록 내역 unavailable 상태
9. 전체 테스트, 빌드, Simulator, Figma 시각 검증
