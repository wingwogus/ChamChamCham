...
**Goal:** Build a fixture-driven medicinal-crop record feedback RAG engine that accepts `TodayRecordFeedbackContext`, while also supporting local smoke tests against real TECH_DOCUMENT chunks in the VectorStore.

**Architecture:** Add a focused `application.coaching.rag.record` package for the record-feedback payload contract, validation, retrieval query planning, prompt construction, and generation service. Keep fake-`VectorStore` unit tests for deterministic behavior, then add a separate local-only seed/smoke path that indexes real medicinal-crop PDF chunks as `TECH_DOCUMENT` and verifies fixture payload retrieval against the actual VectorStore. Do not modify `domain.farming`, farming-record controllers, or work-type field persistence in this plan.
...
This plan implements only the RAG-side contract engine and local document
smoke path:
...
- Unit tests that run without farming-record DB setup.
- Local seed/ingestion of actual PDF corpus chunks as `TECH_DOCUMENT`.
- Local smoke tests where record payload is fake fixture data but document
  retrieval uses the real VectorStore.
- Optional manual OpenClaw e2e for local operator verification.
...
Keep these test layers distinct:

- **Unit:** Uses `TodayRecordFeedbackContext` JSON fixtures and fake
  `VectorStore`/fake `ChatClient`. These tests prove payload parsing, context
  validation, query planning, prompt contents, and structured-output audit.
- **Local smoke:** Uses the same fake record payload fixtures, but seeds real
  official PDF chunks into the Spring AI PgVector `VectorStore` as
  `sourceType = TECH_DOCUMENT`. These tests prove local embedding/vector search
  can find real document chunks for record-feedback queries.
- **Optional manual e2e:** Uses fake record payload fixture, real VectorStore,
  and real OpenClaw `ChatClient`. This is operator-run only and must not be part
  of the normal unit test suite.
- **Future DB assembler integration:** Later, after the record schema is stable,
  test only `FarmingRecord -> TodayRecordFeedbackContext` assembly and keep the
  RAG engine tests reusable.
...
- `backend/api/src/test/kotlin/com/chamchamcham/api/coaching/TodayRecordFeedbackVectorStoreSmokeTest.kt`
...
- `backend/application/src/main/kotlin/com/chamchamcham/application/coaching/rag/seed/DevRagSeedService.kt`
  - Add PDF-only / TECH_DOCUMENT-only seed mode without relational seed.
- `backend/application/src/test/kotlin/com/chamchamcham/application/coaching/rag/seed/DevRagSeedServiceTest.kt`
  - Add coverage that PDF-only seed does not run relational seed SQL.
- `backend/api/src/main/kotlin/com/chamchamcham/api/dev/dto/DevRagSeedRequests.kt`
  - Add request flag for relational seeding control.
- `backend/api/src/main/kotlin/com/chamchamcham/api/dev/dto/DevRagSeedResponses.kt`
  - Allow local PDF-only seed response without issuing a member access token.
- `backend/api/src/main/kotlin/com/chamchamcham/api/dev/controller/DevRagSeedController.kt`
  - Return `accessToken = null` when no relational member was seeded.
...
## Task 7: Add TECH_DOCUMENT-Only Local PDF Seed Mode
...
- Modify: `backend/api/src/main/kotlin/com/chamchamcham/api/dev/dto/DevRagSeedRequests.kt`
- Modify: `backend/api/src/main/kotlin/com/chamchamcham/api/dev/dto/DevRagSeedResponses.kt`
- Modify: `backend/api/src/main/kotlin/com/chamchamcham/api/dev/controller/DevRagSeedController.kt`

- [ ] **Step 1: Add failing PDF-only seed tests**

Append these tests to `backend/application/src/test/kotlin/com/chamchamcham/application/coaching/rag/seed/DevRagSeedServiceTest.kt`:

```kotlin
    @Test
    fun `seed can index pdf chunks without relational seed`() {
        val seedDirectory = Files.createTempDirectory("godsmove-rag-seed-dir-")
        val pdf = Files.createTempFile(seedDirectory, "rda-guide-", ".pdf")
        ...
        try {
            val service = service(
                ...
                vectorStore = vectorStore,
                pdfTextExtractor = FixedPdfTextExtractor(
                    """
                    참당귀 물주기 재배 관리 약용작물
                    고온 건조 시 토양 수분 상태를 확인하고 관수한다.
                    병해충 발생 여부를 함께 관찰한다.
                    """.trimIndent()
                ),
            ...
            assertThat(result.relationalSeeded).isFalse()
            assertThat(result.pdfChunksIndexed).isGreaterThan(0)
            assertThat(result.farmingRecordChunksIndexed).isEqualTo(0)
            assertThat(jdbcTemplate.updates).noneMatch { it.sql.contains("insert into member") }
            assertThat(jdbcTemplate.updates).noneMatch { it.sql.contains("insert into farming_record") }
            assertThat(vectorStore.addCalls).isEqualTo(1)
            assertThat(vectorStore.documents).allSatisfy { document ->
                assertThat(document.metadata["sourceType"]).isEqualTo("TECH_DOCUMENT")
                assertThat(document.metadata["seedName"]).isEqualTo("local-rag-demo")
            }
        } finally {
    ...
    @Test
    fun `seed can index pdf corpus directory as tech documents without relational seed`() {
        val seedDirectory = Files.createTempDirectory("godsmove-rag-seed-dir-")
        val corpusDirectory = Files.createDirectory(seedDirectory.resolve("raw-pdfs"))
        val firstPdf = Files.createTempFile(corpusDirectory, "first-", ".pdf")
        val secondPdf = Files.createTempFile(corpusDirectory, "second-", ".pdf")
        val jdbcTemplate = RecordingJdbcTemplate()
        val vectorStore = RecordingVectorStore()
        ...
        try {
            ...
            val result = service.seed(
                DevRagSeedCommand(
                    pdfPath = seedDirectory.relativize(corpusDirectory).toString(),
                    resetIndex = true,
                    includePdf = true,
                    includeFarmingRecords = false,
                    includeRelationalData = false,
            ...
            assertThat(result.relationalSeeded).isFalse()
            assertThat(result.pdfChunksIndexed).isEqualTo(vectorStore.documents.size)
            assertThat(vectorStore.documents).extracting<String> { it.metadata["sourceId"].toString() }
                .contains(
                    firstPdf.fileName.toString().substringBeforeLast('.'),
                    secondPdf.fileName.toString().substringBeforeLast('.')
                )
            assertThat(jdbcTemplate.updates).noneMatch { it.sql.contains("insert into member") }
            assertThat(jdbcTemplate.updates).noneMatch { it.sql.contains("insert into farming_record") }
        } finally {
            Files.deleteIfExists(firstPdf)
            Files.deleteIfExists(secondPdf)
            Files.deleteIfExists(corpusDirectory)
            Files.deleteIfExists(seedDirectory)
        }
    }

    @Test
    fun `seed rejects farming record chunks when relational seed is disabled`() {
        val service = service()
        ...
        assertThatThrownBy {
            ...
            .isInstanceOfSatisfying(BusinessException::class.java) { exception ->
                assertThat(exception.detail.toString())
                    .contains("includeFarmingRecords requires includeRelationalData")
            }
    }
```

Update the local `RecordingVectorStore` in that test file so it stores documents:
...
Add this extractor test double near the existing extractor test doubles:

```kotlin
    private class FixedPdfTextExtractor(
...
```bash
./gradlew :application:test --tests "com.chamchamcham.application.coaching.rag.seed.DevRagSeedServiceTest"
```

Expected: compilation fails because `includeRelationalData`,
`relationalSeeded`, and directory PDF corpus resolution do not exist.

- [ ] **Step 3: Add seed command/result flags**

In `backend/application/src/main/kotlin/com/chamchamcham/application/coaching/rag/seed/DevRagSeedService.kt`,
change `DevRagSeedCommand` and `DevRagSeedResult` to:
...
data class DevRagSeedCommand(
    ...
    val includeRelationalData: Boolean = true,
...
- [ ] **Step 4: Support file or directory PDF corpus input**

In `DevRagSeedService.seed`, replace the single `ExtractedPdfSeed` preflight
with a list:

```kotlin
        val pdfSeeds = if (command.includePdf) {
            val rawPath = command.pdfPath?.takeIf { it.isNotBlank() }
                ?: throw invalidSeedRequest("pdfPath is required when includePdf is true")
            resolveSeedPdfPaths(rawPath).map { path ->
                ExtractedPdfSeed(
                    sourceId = sourceIdForSeedPdf(path),
                    title = titleForSeedPdf(path),
                    path = path.toString(),
...
Replace `resolveSeedPdfPath` with `resolveSeedPdfPaths`:

```kotlin
    private fun resolveSeedPdfPaths(pdfPath: String): List<Path> {
        ...
        return when {
            Files.isRegularFile(realPath) -> listOf(realPath)
            Files.isDirectory(realPath) -> Files.list(realPath).use { stream ->
                stream
                    .filter { Files.isRegularFile(it) }
                    .filter { it.fileName.toString().endsWith(".pdf", ignoreCase = true) }
                    .sorted()
                    .toList()
            }
            else -> throw invalidSeedRequest("PDF file does not exist")
        }
    }
    ...
    private fun sourceIdForSeedPdf(path: Path): String {
        return fileNameWithoutExtension(path)
    }

    private fun titleForSeedPdf(path: Path): String {
        return fileNameWithoutExtension(path).replace('_', ' ')
    }
```

and call `fileNameWithoutExtension(path)` from `sourceIdForSeedPdf` and
`titleForSeedPdf`.

- [ ] **Step 5: Gate relational and farming-record seed work**

In `DevRagSeedService.seed`, replace the unconditional relational seed and
result creation with this shape:

```kotlin
        if (!command.includeRelationalData && command.includeFarmingRecords) {
            throw invalidSeedRequest("includeFarmingRecords requires includeRelationalData")
        }
        ...
        if (command.includeRelationalData) {
            seedRelationalData()
        }
        ...
        val pdfChunksIndexed = if (command.includePdf) {
            pdfSeeds.sumOf { seedPdfChunks(it, command.maxPdfChunks) }
        } else {
            0
        }

        return DevRagSeedResult(
            memberId = SeedIds.MEMBER_ID.takeIf { command.includeRelationalData },
            farmId = SeedIds.FARM_ID.takeIf { command.includeRelationalData },
            cropId = SeedIds.CROP_ID.takeIf { command.includeRelationalData },
            workTypeIds = if (command.includeRelationalData) {
                SeedRecords.workTypes.mapValues { it.value.id }
            } else {
                emptyMap()
            },
            recordIds = if (command.includeRelationalData) {
                SeedRecords.records.map { it.id }
            } else {
                emptyList()
            },
            relationalSeeded = command.includeRelationalData,
...
Keep the existing preflight behavior that validates/extracts PDF text before
resetting or seeding. Do not move PDF validation after relational writes.

- [ ] **Step 6: Use per-PDF TECH_DOCUMENT metadata**

Change `ExtractedPdfSeed` to:
...
Update `seedPdfChunks` so each PDF gets stable per-file IDs and metadata:
        ...
        val documents = chunks.mapIndexed { index, content ->
            Document(
                ...
                mapOf(
                    ...
                    "sourceId" to pdfSeed.sourceId,
                    "label" to "${pdfSeed.title} ${index + 1}",
                    "documentTitle" to pdfSeed.title,
...
- [ ] **Step 7: Add request/response support for PDF-only seed**

In `backend/api/src/main/kotlin/com/chamchamcham/api/dev/dto/DevRagSeedRequests.kt`,
add `includeRelationalData` with a default of `true`:

```kotlin
        val includeRelationalData: Boolean = true,
```

and pass it into `DevRagSeedCommand`:

```kotlin
                includeRelationalData = includeRelationalData,
```

In `backend/api/src/main/kotlin/com/chamchamcham/api/dev/dto/DevRagSeedResponses.kt`,
make seed IDs and token nullable and include `relationalSeeded`:
        ...
        companion object {
            fun from(result: DevRagSeedResult, accessToken: String?): SeedResponse {
                return SeedResponse(
                    ...
                    relationalSeeded = result.relationalSeeded,
...
In `backend/api/src/main/kotlin/com/chamchamcham/api/dev/controller/DevRagSeedController.kt`,
create an access token only when relational seed returned a member id:

```kotlin
        val accessToken = result.memberId?.let { memberId ->
            tokenProvider.createAccessToken(memberId, "ROLE_USER")
        }
```

Keep returning `ApiResponse.ok(DevRagSeedResponses.SeedResponse.from(result, accessToken))`.

- [ ] **Step 8: Run seed tests and verify they pass**
...
- [ ] **Step 9: Run API compile for DTO/controller nullability**
...
- [ ] **Step 10: Commit PDF-only seed mode**

Run from repository root:

```bash
git add backend/application/src/main/kotlin/com/chamchamcham/application/coaching/rag/seed/DevRagSeedService.kt backend/application/src/test/kotlin/com/chamchamcham/application/coaching/rag/seed/DevRagSeedServiceTest.kt backend/api/src/main/kotlin/com/chamchamcham/api/dev/dto/DevRagSeedRequests.kt backend/api/src/main/kotlin/com/chamchamcham/api/dev/dto/DevRagSeedResponses.kt backend/api/src/main/kotlin/com/chamchamcham/api/dev/controller/DevRagSeedController.kt
git commit -m "feat(rag): 문서 전용 로컬 시드 옵션 추가" -m "로컬 smoke 테스트에서 기록 DB를 만들지 않고 공식 PDF chunk만 TECH_DOCUMENT로 색인할 수 있도록 relational seed를 선택 가능하게 한다." -m "Constraint: 기록 DB와 domain.farming은 이 계획 범위에서 건드리지 않음" -m "Confidence: medium" -m "Scope-risk: moderate" -m "Tested: ./gradlew :application:test --tests \"com.chamchamcham.application.coaching.rag.seed.DevRagSeedServiceTest\"; ./gradlew :api:compileKotlin"
```

## Task 8: Add Local VectorStore Smoke Test

**Files:**

- Create: `backend/api/src/test/kotlin/com/chamchamcham/api/coaching/TodayRecordFeedbackVectorStoreSmokeTest.kt`

- [ ] **Step 1: Write a disabled-by-default local smoke test**
...
- [ ] **Step 2: Compile the smoke test without running it**

Run from `/Users/wingwogus/Projects/ChamChamCham/backend`:

```bash
./gradlew :api:testClasses
```

Expected: `BUILD SUCCESSFUL`. The smoke test must not run unless
`RUN_LOCAL_RAG_SMOKE=true` is set.

- [ ] **Step 3: Document local smoke seed command inside the plan**

Use this exact local operator sequence when running the smoke manually:

```bash
cd /Users/wingwogus/Projects/ChamChamCham/backend
./gradlew :api:bootRun --args='--spring.profiles.active=local --app.dev.rag-seed-dir=/Users/wingwogus/Projects/ChamChamCham/data/rag'
```

Then seed only TECH_DOCUMENT chunks from the actual downloaded PDF corpus
directory. `app.dev.rag-seed-dir` must point at
`/Users/wingwogus/Projects/ChamChamCham/data/rag`.

```bash
curl -sS -X POST http://localhost:8080/api/v1/dev/rag/seed \
  -H 'Content-Type: application/json' \
  -H "X-GodsMove-Dev-Seed-Key: $GODSMOVE_DEV_RAG_SEED_KEY" \
  -d '{
    "pdfPath": "medicinal-plants/raw/pdfs",
    "resetIndex": true,
    "includePdf": true,
    "includeFarmingRecords": false,
    "includeRelationalData": false,
...
```bash
RUN_LOCAL_RAG_SMOKE=true ./gradlew :api:test --tests "com.chamchamcham.api.coaching.TodayRecordFeedbackVectorStoreSmokeTest"
```

Expected: `BUILD SUCCESSFUL`, with non-empty retrieved `TECH_DOCUMENT` chunks.

- [ ] **Step 4: Commit local smoke test**

Run from repository root:

```bash
git add backend/api/src/test/kotlin/com/chamchamcham/api/coaching/TodayRecordFeedbackVectorStoreSmokeTest.kt
git commit -m "test(rag): 실제 VectorStore 기록 피드백 smoke 추가" -m "기록 payload는 fixture로 두고 공식 PDF chunk 검색은 실제 VectorStore로 검증하는 local-only smoke 테스트를 추가한다." -m "Constraint: RUN_LOCAL_RAG_SMOKE=true일 때만 실행되며 CI unit test에는 포함하지 않음" -m "Confidence: medium" -m "Scope-risk: narrow" -m "Tested: ./gradlew :api:testClasses"
```

## Task 9: Optional Manual OpenClaw E2E Procedure

**Files:**

- No planned file edits.

- [ ] **Step 1: Keep this procedure manual-only**

Do not add an always-on integration test for OpenClaw. Real provider calls must
remain manual because they require local secrets, local vector-store state, and
network/provider availability.

- [ ] **Step 2: Use this exact manual sequence**

After Task 8 smoke retrieval passes, run the local app with OpenClaw and the
local VectorStore configured:
...
Then verify the real provider path by using the existing interactive RAG
endpoint after seeding TECH_DOCUMENT chunks:

```bash
curl -sS -X POST http://localhost:8080/api/v1/coaching/rag/query \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $LOCAL_RAG_ACCESS_TOKEN" \
  -d '{
    "question": "참당귀 76일차에 최근 7일 강수량이 적고 오늘 점적 관수를 했는데 다음 기록에서 무엇을 확인해야 하나?",
    "mode": "CHAT",
    "topK": 6
  }'
```

Expected manual e2e result:

- HTTP 200.
- `result.riskLevel` is one of `LOW`, `MEDIUM`, `HIGH`, or `UNKNOWN`.
- `result.recommendations` and `result.nextActions` are present or the result
  clearly says evidence is insufficient.
- Citations refer only to retrieved `TECH_DOCUMENT` chunk ids.

Do not commit provider responses or credentials.
...
- [ ] **Step 5: Verify local smoke remains opt-in**

Run from `/Users/wingwogus/Projects/ChamChamCham/backend` without the smoke
environment variable:

```bash
./gradlew :api:test --tests "com.chamchamcham.api.coaching.TodayRecordFeedbackVectorStoreSmokeTest"
```

Expected: `BUILD SUCCESSFUL` with the smoke test skipped by
...
- fake `VectorStore` unit tests retained: Tasks 3, 4, and 6.
- actual PDF corpus TECH_DOCUMENT seed path: Task 7.
- PDF-only / TECH_DOCUMENT-only seed option when relational seed is normally executed: Task 7.
- fake `TodayRecordFeedbackContext` fixture plus actual VectorStore smoke test: Task 8.
- optional real ChatClient/OpenClaw manual e2e: Task 9.
- test-layer separation for unit, local smoke, optional manual e2e, and future DB assembler integration: Test Layers section.
- no farming-record write model changes: Scope Check and Task 11.
...
- later `record -> payload assembler` split: Scope Check, Test Layers, and Task 11.
...
- The only conditional commit step is Task 10 Step 4, and it explicitly says not to create an empty commit when no files changed.
...
- Local smoke test is isolated in the API test module because it needs real Spring Boot `VectorStore` wiring.
