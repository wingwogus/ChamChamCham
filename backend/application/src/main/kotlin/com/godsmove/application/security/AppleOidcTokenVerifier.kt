...
class AppleOidcTokenVerifier private constructor(
    ...
    fun verify(identityToken: String, rawNonce: String): AppleOidcClaims {
        ...
        val jwt = try {
            ...
            logger.error(exception) { "Apple OIDC decoder failed" }
            throw BusinessException(ErrorCode.INVALID_APPLE_TOKEN)
        }
        ...
        return AppleOidcClaims(
            ...
            emailVerified = emailVerified(jwt.getClaim(EMAIL_VERIFIED_CLAIM)),
            nonce = actualNonce,
            expiresAt = expiresAt
        )
    }
    ...
    private fun emailVerified(value: Any?): Boolean {
