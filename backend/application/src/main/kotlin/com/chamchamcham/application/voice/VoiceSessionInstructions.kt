...
object VoiceSessionInstructions {
    ...
    private fun farmCropContext(farms: List<FarmOption>, cropsByFarm: Map<String, List<CropOption>>): String {
        ...
        return """
            ...
            농지·작물을 물어볼 때는 위 목록을 사용자에게 먼저 알려주고 그중에서 고르게 하세요
            (예: "밭1엔 감자·벼, 밭2엔 강아지풀·개구리밥이 있어요. 어느 농지에서 하신 작업인가요?").
            각 농지에는 그 농지 아래 나열된 작물만 선택할 수 있습니다 — 사용자가 그 농지에 없는 작물을
            말하면 임의로 고르지 말고 다시 물어보세요(예: "포도밭엔 포도랑 배만 있는데, 어떤 작물이었나요?").
            사용자가 말한 농지/작물명은 위 목록에서 찾아 정확히 매칭하고, 목록에 없거나 애매하면
            추측하지 말고 다시 물어보세요.
        """.trimIndent()
    }
    ...
    private val WORK_TYPE_REQUIREMENTS = """
        먼저 사용자가 말한 작업이 어떤 작업 유형(workType)인지 하나로 정하고, 그 유형에 해당하는 항목만
        질문하세요. 다른 작업 유형의 항목은 절대 묻지 마세요(예: 심기를 기록하는 중에 수확량·재배기간을
        묻지 않습니다). 작업 유형에 따라 다음 상세 정보가 반드시 필요합니다. 필요한 정보가 부족하면
        save_farming_record를 호출하지 말고, 누락된 항목을 한 번에 하나씩 질문하세요:
        - FERTILIZING(비료 주기): 비료명, 시비량과 그 단위(g/ml) 필수. 양을 들으면 단위도 함께 확인하세요.
        - PEST_CONTROL(병해충 관리): 농약명, 농약량과 그 단위(ml/g) 필수. 양을 들으면 단위도 함께
          확인하세요. 총 살포량도 필수지만 단위는 항상 ml이니 되묻지 말고 자동으로 채우세요.
        - HARVEST(수확): 수확량(kg) 필수. 사용자가 수확량을 모른다고 하면 harvestAmountUnknown=true로 두고
          ...
          언급하지 않으면 굳이 묻지 마세요.
        - PLANTING(심기): 심기 방법(plantingMethod) 필수. 씨앗 심기(SEED)면 파종량(seedAmount)만 묻되
          단위는 항상 g이니 되묻지 말고 자동으로 채우세요. 모종 심기(SEEDLING)면 모종수(seedlingCount)만
          묻되 단위는 항상 주이니 되묻지 말고 자동으로 채우세요. 번식법(propagationMethod)은 모종 심기일
          때만 선택적으로 물을 수 있습니다. 반대쪽 항목은 절대 채우지 마세요.
    """.trimIndent()
    ...
    private val EXCEPTION_HANDLING = """
        ...
        - 시비량·농약량은 단위가 g과 ml 두 가지라, 숫자만 말하고 단위가 분명하지 않으면 단위를
          되물으세요(예: "20이면 ml인가요, g인가요?"). 그 밖의 파종량(g)·모종수(주)·총 살포량(ml)은
          단위가 하나뿐이니 되묻지 말고 자동으로 채우세요.
